...
파이썬 프로세스를 잠시 중단시키기 위해 time.sleep() 을 사용해 본 경험이 있을 것이다.
...
자 이제 기존의 sleep() 은 한 켠에 제쳐두고 파이썬 3.7부터 도입된
새로운 비동기 시스템에 대해서 알아보자.
...
async 함수도 이와 마찬가지로, async 함수를 호출하면 함수가 바로 실행되는 것이 아닌
"코루틴"을 넘겨받게 된다. 
...
가지고 빠져나가는 것을 기억하는가? `await` 은 async 함수 내부에서 `yield` 와 같은 기능을 한다.
따라서 time.sleep() 으로 한 쓰레드가 잠들어 있는 동안, 파이썬은 다른 함수를 실행할 수 있게 된다.


## async 와 await 으로 동시에 2개의 함수 실행하기 

이제는 직접 함수를 2번 실행해서, 진짜 두 함수가 동시에 시작하는지를 테스트 해보도록 하자.
